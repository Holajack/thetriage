 ...
 */
export const resetUserData = internalMutation({
  args: { clerkId: v.string() },
  handler: async (ctx, args) => {
    const user = await ctx.db
      .query("users")
 ...
 */
export const seedFakeFriends = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!adminUser) {
      throw new Error(
        `Admin user not found with Clerk ID: ${args.adminClerkId}`,
      );
    }
    ...
    const fakeFriends = [
      {
        firstName: "Marcus",
        lastName: "Chen",
        username: "mchen_study",
        email: "marcus.c@demo.thetriage.app",
      },
      {
        firstName: "Sofia",
        lastName: "Rodriguez",
        username: "sofi_r",
        email: "sofia.r@demo.thetriage.app",
      },
      {
        firstName: "James",
        lastName: "Okonkwo",
        username: "jamesO",
        email: "james.o@demo.thetriage.app",
      },
      {
        firstName: "Emma",
        lastName: "Patel",
        username: "emma.patel",
        email: "emma.p@demo.thetriage.app",
      },
      {
        firstName: "Lucas",
        lastName: "Kim",
        username: "lucask22",
        email: "lucas.k@demo.thetriage.app",
      },
      {
        firstName: "Ava",
        lastName: "Thompson",
        username: "avathompson",
        email: "ava.t@demo.thetriage.app",
      },
      {
        firstName: "Noah",
        lastName: "Martinez",
        username: "noahm",
        email: "noah.m@demo.thetriage.app",
      },
    ];
 ...
 */
export const cleanupFakeFriends = internalMutation({
  args: {},
  handler: async (ctx) => {
    let deleted = 0;
    ...
    return {
      success: true,
      deleted,
      message: `Cleaned up ${deleted} demo friends`,
    };
  },
});
 ...
 * Usage: Call from Convex Dashboard with { adminClerkId: "user_XXXX" }
 */
export const seedFakeMessages = internalMutation({
  args: { adminClerkId: v.string() },
  handler: async (ctx, args) => {
    // Find admin user
    const adminUser = await ctx.db
    ...
    const conversations = [
      {
        ...
        messages: [
          {
            fromFriend: true,
            content: "Hey! Ready for the study session?",
            hoursAgo: 4,
            isRead: true,
          },
          {
            fromFriend: false,
            content: "Yes! Let me grab my notes",
            hoursAgo: 3.9,
            isRead: true,
          },
          {
            fromFriend: true,
            content: "Cool, I'll start the room. Join when ready!",
            hoursAgo: 3.8,
            isRead: true,
          },
          {
            fromFriend: false,
            content: "Just joined. This calculus chapter is rough 😅",
            hoursAgo: 3.5,
            isRead: true,
          },
          {
            fromFriend: true,
            content: "We got this! 45 min sprint then break?",
            hoursAgo: 3.4,
            isRead: true,
          },
        ],
      },
      {
        ...
        messages: [
          ...
          {
            fromFriend: false,
            content: "Almost done! Problem 5 is tricky",
            hoursAgo: 47,
            isRead: true,
          },
          {
            fromFriend: true,
            content: "I can help if you want to study together",
            hoursAgo: 46,
            isRead: true,
          },
          {
          ...
          {
            fromFriend: true,
            content: "Want to start a session tomorrow at 2pm?",
            hoursAgo: 2,
            isRead: false,
          },
          {
            fromFriend: true,
            content: "I found some good practice problems we can work through",
            hoursAgo: 1.5,
            isRead: false,
          },
        ],
      },
      {
        ...
        messages: [
          ...
          {
            fromFriend: false,
            content: "Thanks! Your streak is impressive",
            hoursAgo: 7.5,
            isRead: true,
          },
          {
          ...
          },
          {
            fromFriend: true,
            content:
              "I set small goals each day. Start with just 25min if needed",
            hoursAgo: 6,
            isRead: true,
          },
          {
            fromFriend: true,
            content: "Also this app helps a lot haha",
            hoursAgo: 1,
            isRead: false,
          },
        ],
      },
      {
        ...
        messages: [
          {
            fromFriend: true,
            content: "Are you taking CHEM 101 too?",
            hoursAgo: 72,
            isRead: true,
          },
          {
            fromFriend: false,
            content: "Yes! The midterm is coming up fast",
            hoursAgo: 70,
            isRead: true,
          },
          {
            fromFriend: true,
            content: "Wanna form a study group?",
            hoursAgo: 68,
            isRead: true,
          },
          {
            fromFriend: false,
            content: "Definitely! I could use the accountability",
            hoursAgo: 66,
            isRead: true,
          },
        ],
      },
      {
        ...
        messages: [
          {
            fromFriend: true,
            content: "Just hit level 5! 🎉",
            hoursAgo: 24,
            isRead: true,
          },
          {
            fromFriend: false,
            content: "Nice! Congrats!",
            hoursAgo: 23,
            isRead: true,
          },
          {
            fromFriend: true,
            content: "Thanks, trying to catch up to you on the leaderboard",
            hoursAgo: 22,
            isRead: true,
          },
        ],
 ...
 * Usage: Call from Convex Dashboard with { adminClerkId: "user_XXXX" }
 */
export const seedFakeStudyRooms = internalMutation({
  args: { adminClerkId: v.string() },
  handler: async (ctx, args) => {
    // Find admin user
    const adminUser = await ctx.db
    ...
    if (!marcus || !sofia) {
      throw new Error(
        "Required fake friends not found. Run seedFakeFriends first.",
      );
    }
    ...
    const calcMessages = [
      {
        sender: marcus,
        content: "Welcome everyone! Let's focus on derivatives today",
        type: "text",
      },
      {
        sender: adminUser,
        content: "Sounds good! I'm on problem 3",
        type: "text",
      },
      {
        sender: sofia,
        content: "Can someone explain the chain rule again?",
        type: "text",
      },
      {
        sender: marcus,
        content: "Sure! It's d/dx[f(g(x))] = f'(g(x)) · g'(x)",
        type: "text",
      },
      {
        sender: sofia,
        content: "That makes more sense now, thanks!",
        type: "text",
      },
      {
        sender: adminUser,
        content: "15 min left in this sprint!",
        type: "text",
      },
    ];
    ...
    const chemMessages = [
      {
        sender: adminUser,
        content: "Let's review reaction mechanisms first",
        type: "text",
      },
      {
        sender: emma || sofia,
        content: "Good idea. SN1 vs SN2 always confuses me",
        type: "text",
      },
      {
        sender: adminUser,
        content: "Me too. Let's make some flashcards for those",
        type: "text",
      },
    ];
