/**
 * Deer Frame Extractor
 *
 * Extracts all 28 walking frames from the deer animation sprite sheet.
 * Uses grid-based extraction with auto-trim to find deer in each cell.
 *
 * Layout: 4 columns x 7 rows = 28 frames
 * Output: 200x200 frames, centered horizontally, anchored at bottom
 *
 * Usage: node scripts/extract-deer-frames.js
 */
...
// Configuration
const INPUT_FILE = path.join(__dirname, '../assets/examples/combined_sprite_bc752f0b-8d22-465c-bd00-7df38fb4c011.png');
const OUTPUT_DIR = path.join(__dirname, '../assets/trail-buddies/deer-frames');
const SPRITE_SHEET_OUTPUT = path.join(__dirname, '../assets/trail-buddies/deer_walking_optimized.png');
const FRAME_DATA_OUTPUT = path.join(__dirname, '../assets/trail-buddies/deer_walking_frames.json');
...
async function main() {
  console.log('Deer Frame Extractor');
  console.log('====================\n');
  ...
  for (let row = 0; row < GRID_ROWS; row++) {
    for (let col = 0; col < GRID_COLS; col++) {
      ...
      // Extract cell, then trim whitespace to find the deer
      ...
      const scaledDeer = await sharp(trimmed.data)
      ...
      const outputPath = path.join(OUTPUT_DIR, `deer_frame_${String(frameIndex).padStart(2, '0')}.png`);
      await sharp({
        create: {
          width: OUTPUT_SIZE,
          height: OUTPUT_SIZE,
        ...
        .composite([{ input: scaledDeer, left, top }])
        .png()
        .toFile(outputPath);
  ...
  // Generate JSON
  const frameData = {
    spriteSheet: 'deer_walking_optimized.png',
