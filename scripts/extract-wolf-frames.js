/**
 * Wolf Frame Extractor
 *
 * Extracts all 28 walking frames from the wolf animation sprite sheet.
 * Uses grid-based extraction with auto-trim to find wolf in each cell.
 *
 * Layout: 4 columns x 7 rows = 28 frames
 * Output: 200x200 frames, centered horizontally, anchored at bottom
 *
 * Usage: node scripts/extract-wolf-frames.js
 */
...
// Configuration
const INPUT_FILE = path.join(__dirname, '../assets/examples/wolf_animation.png');
const OUTPUT_DIR = path.join(__dirname, '../assets/trail-buddies/wolf-frames');
const SPRITE_SHEET_OUTPUT = path.join(__dirname, '../assets/trail-buddies/wolf_walking_optimized.png');
const FRAME_DATA_OUTPUT = path.join(__dirname, '../assets/trail-buddies/wolf_walking_frames.json');
...
async function main() {
  console.log('Wolf Frame Extractor');
  console.log('====================\n');
  ...
  for (let row = 0; row < GRID_ROWS; row++) {
    for (let col = 0; col < GRID_COLS; col++) {
      ...
      // Extract cell, then trim whitespace to find the wolf
      ...
      const scaledWolf = await sharp(trimmed.data)
      ...
      const outputPath = path.join(OUTPUT_DIR, `wolf_frame_${String(frameIndex).padStart(2, '0')}.png`);
      await sharp({
        create: {
          width: OUTPUT_SIZE,
          height: OUTPUT_SIZE,
        ...
        .composite([{ input: scaledWolf, left, top }])
        .png()
        .toFile(outputPath);
  ...
  // Generate JSON
  const frameData = {
    spriteSheet: 'wolf_walking_optimized.png',
