...
const FloatingNoraContext = createContext<FloatingNoraContextType | undefined>(
  undefined,
);

export const FloatingNoraProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const shouldShowNora = (screenName: string = currentScreen) => {
    ...
    const isHomeScreen =
      screenName === "Home" || screenName.toLowerCase().includes("home");
    const isOtherRestrictedScreen = restrictedScreens
      .slice(0, -1)
      .some((screen) => screenName.includes(screen));
  ...
  const handleNavigateToNora = (messages: any[]) => {
    if (navigationInstance) {
      try {
        ...
        // Error navigating to Nora screen
      }
    }
  };
...
export const useFloatingNora = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useFloatingNora must be used within a FloatingNoraProvider",
    );
  }
  return context;
};
