...
export default function LoginScreen() {
      ...
      if (sessionId) {
        try {
          ...
          if (
            ...
            // Expected RN compat error — session is still activated
          }
        }
      }
      ...
      if (signInAttempt.status === "complete") {
        ...
        if (strategies.includes("password")) {
          ...
          try {
            ...
            const msg =
              passwordErr?.errors?.[0]?.longMessage ||
              passwordErr?.errors?.[0]?.message ||
              passwordErr?.message ||
              "Incorrect password.";
            setLoginError(msg);
            triggerErrorShake();
        ...
        setLoginError(
          'Your password needs to be reset. Use "Forgot Password" below.',
        );
        triggerErrorShake();
      } else {
  ...
  const inputBorderColor = theme.isDark
    ? "rgba(255, 255, 255, 0.2)"
    : "rgba(232, 245, 233, 0.3)";
  const inputBackground = theme.isDark
...
const styles = StyleSheet.create({
  ...
  errorText: {
    // Renamed from error to errorText for clarity, ensure this is used or styles.error is suitable
    color: "#FF6B6B",
    marginBottom: 12,
    textAlign: "center",
