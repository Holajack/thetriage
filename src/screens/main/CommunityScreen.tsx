    ...
    >
      ...
      >
        ...
        <View style={{ alignItems: "flex-end" }}>
          <Text style={[styles.timeText, { color: theme.text + "99" }]}>
            {timeAgo}
          </Text>
          {item.unreadCount > 0 && (
            <View
...
const CommunityScreen = () => {
  ...
  const [showFriendRequestNotification, setShowFriendRequestNotification] =
    useState(false);
  const [showMessageNotification, setShowMessageNotification] = useState(false);
  const [showStudyRoomInvitations, setShowStudyRoomInvitations] =
    useState(false);
  const [pendingStudyRoomInvitations, setPendingStudyRoomInvitations] =
    useState<StudyRoomService.StudyRoomInvitation[]>([]);
  ...
  const handleCreateRoom = async () => {
    ...
    if (!newRoomName.trim() || !newTopic.trim()) {
      Alert.alert(
        "Validation Error",
        "Please fill in the room name and topic.",
      );
      return;
    }
  ...
  const { conversations, loading: conversationsLoading } =
    useConvexConversations(currentUser);
  const { users: allUsers, loading: allUsersLoading } =
    useConvexAllUsers(currentUser);
  const {
    rooms: roomsData,
    loading: studyRoomsLoading,
  ...
  const getFriendStatus = (userId: string) => {
    ...
    const legacyRequest = friendRequests.find(
      (r) =>
        ("sender_id" in r
          ? (r as any).sender_id === currentUser.id &&
            (r as any).recipient_id === userId
          : (r as any).user_id === currentUser.id &&
            (r as any).friend_id === userId) ||
        ("sender_id" in r
          ? (r as any).sender_id === userId &&
            (r as any).recipient_id === currentUser.id
          : (r as any).user_id === userId &&
            (r as any).friend_id === currentUser.id),
    );
    ...
    if ("sender_id" in legacyRequest) {
      return (legacyRequest as any).sender_id === currentUser.id
        ? "pending"
        : "incoming";
    }

    return (legacyRequest as any).status || null;
  };
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (i === retries - 1) {
          // Last attempt failed
          throw new Error(
            `Network operation failed after ${retries} attempts: ${error.message}`,
          );
        }
    ...
    <View style={{ flex: 1 }}>
        ...
        <SafeAreaView style={styles.container}>
                  ...
                  return (
                    ...
                    >
                        ...
                        >
                          {user.avatar_url ? (
                            <Image
                              ...
                              style={styles.avatarCircleCompact}
                            />
                          ) : (
                            <Text
                              style={[
                      ...
                      <View style={styles.actionButtonsCompact}>
                        ...
                        {!friendStatus &&
                          !pendingFriendRequestIds.includes(user.id) && (
                            <TouchableOpacity
                        ...
                        {friendStatus === "incoming" &&
                          incomingRequestForUser && (
                            <View style={{ flexDirection: "row" }}>
                              <TouchableOpacity
                                style={styles.addFriendButtonCompact}
                                onPress={() =>
                                  handleAcceptFriendRequest(
                                    incomingRequestForUser.id,
                                  )
                                }
                              >
                                <Ionicons
                        ...
                        {friendStatus === "pending" && (
                          <View style={styles.pendingBadgeCompact}>
                            <Text style={styles.pendingTextCompact}>
                              Pending
                            </Text>
                          </View>
                        )}
          ...
          {!search.trim() && (
            <View
              ref={tabBarRef}
              collapsable={false}
              style={[styles.tabRow, { backgroundColor: theme.card }]}
            >
              {TABS.map((tab) => (
                <TouchableOpacity
                  key={tab}
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        color:
                          activeTab === tab ? theme.primary : theme.text + "99",
                      },
                    ]}
                  >
                    {tab}
                      ...
                      >
                        <View style={styles.avatarCircle}>
                          {(senderProfile as any).avatar_url ? (
                            <Image
                              source={{
                                uri: (senderProfile as any).avatar_url,
                              }}
                              style={styles.avatarCircle}
                            />
                          ) : (
                  ...
                  <AnimatedFriendCard
                    ...
                    onMessage={(friendItem: any) =>
                      navigation.navigate("MessageScreen" as any, {
                        contact: {
                          id:
                            friendItem.friend_profile?.id ||
                            friendItem.friend_id,
                          name:
                            friendItem.friend_profile?.full_name ||
                            friendItem.friend_profile?.username ||
                            "Study Friend",
                          avatar: friendItem.friend_profile?.avatar_url,
                          status: getStatusLabel(
                            friendItem.friend_profile?.status,
                          ),
                        },
                      })
                    }
          ...
          {!search.trim() && activeTab === "Study Rooms" && (
            <ScrollView
              ref={studyRoomsRef as any}
              collapsable={false}
              key={`studyrooms-${focusKey}`}
              contentContainerStyle={{ paddingBottom: 24 }}
            >
              <TouchableOpacity
                style={[
              ...
              >
                ...
                <Text style={[styles.createRoomBtnText, { color: theme.text }]}>
                  Create Study Room
                </Text>
              </TouchableOpacity>
              ...
              {studyRooms.length === 0 ? (
                <View style={styles.emptyContainer}>
                  ...
                  <Text
                    style={[styles.emptySubtext, { color: theme.text + "99" }]}
                  >
                    Create or join a study room to start collaborating!
                  </Text>
                </View>
              ) : (
                studyRooms.map((room) => (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          Create a new study room to collaborate with other
                          students
                        </Text>
                        ...
                        <TouchableOpacity
                          style={[
                            styles.modalCreateRoomBtn,
                            (!newRoomName.trim() || !newTopic.trim()) &&
                              styles.disabledButton,
                          ]}
                          onPress={handleCreateRoom}
                          disabled={!newRoomName.trim() || !newTopic.trim()}
                        >
                          <Text style={styles.modalCreateRoomBtnText}>
                            Create Room
                          </Text>
                        </TouchableOpacity>

                        <TouchableOpacity
                          style={styles.modalCancelBtnLarge}
                          onPress={() => setShowCreateModal(false)}
                        >
                          <Text style={styles.modalCancelTextLarge}>
                            Cancel
                          </Text>
                        </TouchableOpacity>
                      </View>
      ...
      {networkStatus !== "online" && (
        ...
        >
          <Ionicons
            name={
              networkStatus === "offline" ? "wifi-outline" : "hourglass-outline"
            }
            size={16}
            color="#fff"
          />
