...
const StudyRoomScreen = () => {
  ...
  const [members, setMembers] = useState<
    StudyRoomService.StudyRoomParticipant[]
  >([]);
  ...
  const isOwner =
    user?.id &&
    roomData &&
    (roomData.owner_id === user.id || (roomData as any).creator_id === user.id);
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const result = await StudyRoomService.removeMemberFromStudyRoom(
                roomData.id,
                memberId,
              );
              if (result.success) {
                ...
                Alert.alert(
                  "Success",
                  `${memberName} has been removed from the study room`,
                );
          ...
          onPress: async () => {
            try {
              ...
              if (result.success) {
                ...
                Alert.alert(
                  "Error",
                  result.error || "Failed to delete study room",
                );
              }
            } catch {
              Alert.alert("Error", "An unexpected error occurred");
  ...
  const handleSendInvitations = async () => {
    ...
    try {
      const promises = selectedFriends.map((friendId: string) =>
        StudyRoomService.sendStudyRoomInvitation(roomData.id, friendId),
      );

      const results = await Promise.all(promises);
      const failedInvites = results.filter((result: any) => !result.success);

      if (failedInvites.length === 0) {
        Alert.alert(
          "Success",
          `Sent ${selectedFriends.length} invitation${selectedFriends.length !== 1 ? "s" : ""}!`,
        );
      } else {
        Alert.alert(
          "Partial Success",
          `Sent ${results.length - failedInvites.length} of ${results.length} invitations`,
        );
      }
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await StudyRoomService.sendStudyRoomMessage(
        roomData.id,
        messageContent,
      );
      if (result.success) {
  ...
  const renderMessage = ({
    ...
    item: StudyRoomService.StudyRoomMessage;
    index: number;
  }) => {
      ...
      <Animated.View
        ...
        style={[
          ...
          isMyMessage
            ...
            : [styles.messageBubbleOther, { backgroundColor: theme.card }],
        ]}
      >
        <Text
          style={[
            styles.messageSender,
            { color: isMyMessage ? "#fff" : theme.primary },
          ]}
        >
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        >
          <View style={[styles.infoCard, { backgroundColor: theme.card }]}>
            ...
            <Text style={[styles.label, { color: theme.text }]}>
              Participants:{" "}
              <Text style={[styles.value, { color: theme.primary }]}>
                {roomData?.current_participants || 1} /{" "}
            ...
            <Text style={[styles.label, { color: theme.text }]}>
              Subject:{" "}
              <Text style={[styles.value, { color: theme.primary }]}>
                {roomData?.subject || "General Study"}
              </Text>
            </Text>
          ...
          <TouchableOpacity
            style={[styles.startSessionBtn, { backgroundColor: theme.primary }]}
            onPress={() =>
        ...
        >
          ...
          >
            {sending ? (
              <ShimmerLoader variant="circle" size={22} />
            ) : (
          ...
          <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
                  ...
                  <View style={styles.friendInfo}>
                    ...
                    >
                      <Text style={styles.avatarText}>
                        {item.friend_profile?.full_name?.[0] ||
                          item.friend_profile?.username?.[0] ||
                          "U"}
                      </Text>
                    </View>
            ...
            >
              <Text style={styles.sendInviteButtonText}>
                Send Invitation{selectedFriends.length !== 1 ? "s" : ""} (
                    ...
                    >
                      ...
                      {!isOwnerMember && (
                        <TouchableOpacity
                          ...
                          onPress={() =>
                            handleRemoveMember(item.user_id, memberName)
                          }
                        >
                          <Ionicons
...
const styles = StyleSheet.create({
  ...
  chatContainerWrapper: {
    flex: 1,
    minHeight: 180,
  } /* Removed maxHeight to allow chat to expand */,
  chatContainer: {
    borderRadius: 16,
    marginHorizontal: 16,
    marginBottom: 16,
