...
const AISettingsScreen = () => {
  ...
  const handleAIToggle = async (aiType: 'nora' | 'patrick' | 'insights', value: boolean) => {
    ...
    if (isPremium && aiType === 'nora') {
      Alert.alert('Elite Feature', 'Nora AI with full contextual access is an Elite-only feature.', [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Upgrade to Elite', onPress: () => navigation.navigate('Subscription') },
      ]);
      return;
    }
    ...
    try {
      ...
      const names = { nora: 'Nora AI', patrick: 'Patrick AI', insights: 'AI Insights' };
      Alert.alert('Updated', `${names[aiType]} ${value ? 'enabled' : 'disabled'}`);
    } catch (error) {
  ...
  const handlePersonalizationToggle = async (value: boolean) => {
    if (!isElite) {
      Alert.alert('Elite Feature', 'Personalized AI responses require an Elite subscription.', [
        { text: 'Cancel', style: 'cancel' },
        { text: 'Upgrade to Elite', onPress: () => navigation.navigate('Subscription') },
      ]);
      return;
    ...
    try {
      if (!clerkUserId) return;
      await updateUserSettings(clerkUserId, { personalized_responses: value } as any);
  ...
  const tierConfig = !hasAIAccess
    ? { label: 'Free Plan', color: '#EF5350', bg: '#FFEBEE', icon: 'lock-closed' as const, desc: 'Upgrade to Premium or Elite to access AI features.' }
    : isElite
    ? { label: 'Elite Plan', color: '#2E7D32', bg: '#E8F5E9', icon: 'star' as const, desc: 'Full access to Nora, Patrick, and AI insights.' }
    : { label: 'Premium Plan', color: '#F57C00', bg: '#FFF3E0', icon: 'sparkles' as const, desc: 'Access to Patrick and basic AI insights. Upgrade to Elite for Nora.' };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>AI Integration</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.content}>
        ...
        <StaggeredItem index={0}>
          <View style={[styles.tierBadge, { backgroundColor: tierConfig.bg, borderLeftColor: tierConfig.color }]}>
            ...
            <View style={{ flex: 1, marginLeft: Spacing.sm }}>
              ...
              <Text style={[styles.tierDesc, { color: tierConfig.color }]}>{tierConfig.desc}</Text>
              {!hasAIAccess && (
                <TouchableOpacity
                  onPress={() => navigation.navigate('Subscription')}
                  style={[styles.upgradeBtn, { backgroundColor: tierConfig.color }]}
                >
        ...
        <StaggeredItem index={2}>
          ...
          <SettingsGroup>
            <SettingsRow
              icon="bulb-outline"
              label="AI Insights"
              description={isElite ? 'Full AI-powered study insights and suggestions' : 'Basic AI insights for study sessions'}
              toggle
