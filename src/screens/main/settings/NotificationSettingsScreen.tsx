...
const NotificationSettingsScreen = () => {
  ...
  const requestNotificationPermission = async () => {
    try {
      ...
      if (status === 'granted') {
        ...
        if (clerkUserId) {
          await updateUserSettings(clerkUserId, { notifications_enabled: true });
        }
      } else {
  ...
  const handleNotificationToggle = async (value: boolean) => {
    if (value && notificationPermission !== 'granted') {
      ...
      try {
        if (clerkUserId) {
          await updateUserSettings(clerkUserId, { notifications_enabled: value });
        }
        if (value && notificationPermission === 'granted') {
  ...
  const handleDailyReminderUpdate = () => {
    Alert.alert('Daily Study Reminder', 'Set your preferred time for daily study reminders', [
      { text: 'Morning (8:00 AM)', onPress: () => updateDailyReminder('08:00') },
      { text: 'Afternoon (2:00 PM)', onPress: () => updateDailyReminder('14:00') },
      { text: 'Evening (6:00 PM)', onPress: () => updateDailyReminder('18:00') },
      {
        text: 'Custom Time',
        onPress: () => {
          Alert.prompt(
            ...
            (time) => {
              ...
              if (match) {
                ...
                updateDailyReminder(`${hours.toString().padStart(2, '0')}:${mins}`);
              } else {
  ...
  const handleSessionEndReminderToggle = async (value: boolean) => {
    ...
    try {
      if (clerkUserId) {
        await updateUserSettings(clerkUserId, { session_end_reminder: value } as any);
      }
    } catch (error) {
        ...
        <StaggeredItem index={1}>
          ...
          <SettingsGroup>
            <SettingsRow
              ...
              description={permissionGranted ? `Scheduled for ${formatTime(dailyReminder)}` : 'Enable notifications first'}
            ...
            >
              {notifWeeklyGoalReminders && (
                <View style={{ marginTop: Spacing.sm }}>
                  <Text style={[styles.dayLabel, { color: theme.textSecondary ?? '#666' }]}>Notify me on:</Text>
                    ...
                    {['Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.dayChip,
                          {
                            backgroundColor: weeklyGoalReminderDays.includes(day) ? theme.primary : (isDark ? '#2f2f2f' : '#F5F5F5'),
                            borderColor: weeklyGoalReminderDays.includes(day) ? theme.primary : borderColor,
                          },
                        ]}
                      >
                        <Text
                          style={[
                            styles.dayChipText,
                            { color: weeklyGoalReminderDays.includes(day) ? '#FFF' : theme.text },
                          ]}
                        >
                          {day.substring(0, 3)}
                        </Text>
