...
let AppleMusicAuth:
  | typeof import("@lomray/react-native-apple-music").Auth
  | null = null;
let AppleMusicPlayer:
  | typeof import("@lomray/react-native-apple-music").Player
  | null = null;
let AppleMusicKit:
  | typeof import("@lomray/react-native-apple-music").MusicKit
  | null = null;
let AppleMusicAuthStatus:
  | typeof import("@lomray/react-native-apple-music").AuthStatus
  | null = null;
...
class AppleMusicService implements MusicProvider {
  ...
  async connect(): Promise<boolean> {
    if (Platform.OS !== "ios") {
      Alert.alert(
        "Apple Music",
        "Apple Music integration is only available on iOS devices.",
      );
      return false;
    }
    ...
    try {
      ...
      const wasConnected = await SecureStore.getItemAsync(
        SECURE_STORE_KEYS.connected,
      );
