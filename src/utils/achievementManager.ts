...
const initializeUserAchievements = async (
  ...
  try {
    ...
    if (existingAchievements.achievements.length === 0) {
      ...
      const initialAchievements: Achievement[] = ACHIEVEMENTS.map(
        (achievement) => ({
          ...achievement,
          currentTier: 0,
          progress: 0,
    ...
    if (achievementsJson) {
      ...
      parsed.achievements = parsed.achievements.map((achievement: any) => ({
        ...
        dateEarned: achievement.dateEarned
          ? new Date(achievement.dateEarned)
          : undefined,
      }));
      return parsed;
    }
...
const saveUserAchievements = async (
  ...
  try {
    ...
    // Failed to persist achievements
  }
};
...
const updateUserProgress = async (
  ...
  try {
    ...
    const { newBadges, updatedAchievements } = checkForNewAchievements(
...
export const recordQuizCompletion = async (
  ...
  try {
    ...
    const currentQuizAchievement = userAchievements.achievements.find(
      (a) => a.id === "quiz_completion",
    );
