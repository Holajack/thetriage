...
const BRAIN_3D_REGIONS = [
  {
    id: "prefrontal_cortex",
    name: "Prefrontal Cortex",
    position: { x: 0, y: 0.8, z: 0.6 },
    description:
      "Executive functions, decision making, and working memory. Critical for planning and problem-solving.",
    defaultColor: "#4CAF50",
  },
  {
    id: "left_parietal",
    ...
    description:
      "Mathematical processing, spatial reasoning, and logical thinking.",
    defaultColor: "#2196F3",
  },
  {
    id: "right_parietal",
  ...
  },
  {
    ...
    description:
      "Language processing, auditory information, and verbal memory.",
    defaultColor: "#FF9800",
  },
  {
    id: "right_temporal",
    ...
    position: { x: 0.9, y: -0.2, z: 0.1 },
    description:
      "Musical processing, pattern recognition, and emotional memory.",
    defaultColor: "#9C27B0",
  },
  {
    id: "occipital_lobe",
    ...
    description:
      "Visual processing, image recognition, and visual-spatial skills.",
    defaultColor: "#E91E63",
  },
  {
    id: "broca_area",
  ...
  },
  {
    ...
    description:
      "Language comprehension, speech understanding, and semantic processing.",
    defaultColor: "#FF5722",
  },
  {
    id: "hippocampus_left",
  ...
  },
  {
    ...
    description:
      "Movement control, fine motor skills, and physical coordination.",
    defaultColor: "#795548",
  },
  {
...
const SUBJECT_BRAIN_MAPPING: { [key: string]: string[] } = {
...
const convertTo3DBrainData = (activities: BrainActivity[]): Brain3DRegion[] => {
  ...
  activities.forEach((activity, index) => {
    ...
    const brainRegionIds = SUBJECT_BRAIN_MAPPING[subjectKey] ||
      SUBJECT_BRAIN_MAPPING[subjectKey.toLowerCase()] || ["prefrontal_cortex"]; // Default fallback

    // Use the first available region for this subject
    const regionId =
      brainRegionIds.find((id) => !assignedRegions.has(id)) ||
      brainRegionIds[0];
...
export const generateBrainVisualizationData = (
  userData: any,
): Brain3DRegion[] => {
  ...
  const subjectActivity: {
    [key: string]: { time: number; lastActive: Date | null; sessions: any[] };
  } = {};
  ...
  tasks.forEach((task: any) => {
    ...
    const taskSessions = sessions.filter(
      (session: any) => session.task_id === task.id,
    );
    ...
    const lastActive = latestSession?.created_at
    ...
    subjectActivity[subject].sessions =
      subjectActivity[subject].sessions.concat(taskSessions);

    if (
      lastActive &&
      (!subjectActivity[subject].lastActive ||
        lastActive > subjectActivity[subject].lastActive)
    ) {
  ...
  const maxTime = Math.max(
    ...Object.values(subjectActivity).map((s) => s.time),
    1,
  );
  const assignedRegions = new Set<string>();
  const brain3DRegions: Brain3DRegion[] = [];
  ...
  Object.entries(subjectActivity).forEach(([subject, data]) => {
    const brainRegionIds = SUBJECT_BRAIN_MAPPING[subject] ||
      SUBJECT_BRAIN_MAPPING[subject.toLowerCase()] || ["prefrontal_cortex"];
    ...
    if (brainRegion) {
      ...
      const colorIndex = Math.floor(
        Math.abs(subject.charCodeAt(0)) % colors.length,
      );
