...
export const getMusicPreferences = async (
  ...
  try {
    ...
    // Error getting music preferences
    return {
...
export const saveMusicPreferences = async (
  userId: string,
  preferences: Partial<MusicPreferences>,
): Promise<void> => {
  try {
    const client = getConvexClient();
    ...
    if (
      preferences.auto_play_sound !== undefined ||
      preferences.music_volume !== undefined
    ) {
    ...
    // Error saving music preferences
    throw error;
  }
};
