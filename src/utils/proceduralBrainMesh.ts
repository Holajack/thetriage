...
const EXTRA_TRIANGLES =
  PROCEDURAL_BRAIN_FACE_COUNT - (VERTEX_ROWS - 1) * (VERTEX_COLS - 1); // 524

const clamp01 = (value: number) => THREE.MathUtils.clamp(value, 0, 1);

export const createProceduralBrainMesh = (activityLevel = 0): any => {
  ...
  for (let row = 0; row < VERTEX_ROWS; row++) {
    for (let col = 0; col < VERTEX_COLS; col++) {
      ...
      const radiusBase =
        42 + 5 * Math.sin(theta) * Math.sin(theta) + 4 * hemisphereBlend;
      const radius =
      ...
      const x =
        radius * sinTheta * cosPhi * (1 + 0.06 * hemisphereBlend) +
        lobeShift * 18;
      const y =
        radius * cosTheta * 0.85 +
        6 * Math.sin(phi * 4) * Math.pow(sinTheta, 2);
      const z =
        radius * sinTheta * sinPhi * (1 - 0.05 * hemisphereBlend) +
        hemisphereBlend * 6;
      ...
      const saturation = clamp01(
        0.55 + 0.25 * foldStrength + 0.15 * activityLevel,
      );
      const lightness = clamp01(
        0.58 + 0.12 * activityLevel - 0.18 * heightNorm,
      );
  ...
  if (indexOffset !== PROCEDURAL_BRAIN_FACE_COUNT * 3) {
    // Procedural brain index generation mismatch
  }
  ...
  const mesh = new THREE.Vector3(geometry, material);
